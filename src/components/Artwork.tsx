...
interface ArtworkProps {
  id: number;
  alt: string;
  /** Box metrics only - the image and its pokéball fallback must both honour them. */
  style?: { width?: number | '100%'; height?: number; marginTop?: number };
  pokemon?: Pokemon;
...
export function Artwork({ id, alt, style, pokemon, placeholderSize = 56, thumbWidth }: ArtworkProps) {
  ...
  if (!source) {
    return (
      <View style={[styles.fallback, style]} accessibilityLabel={`${alt}, no artwork available`}>
