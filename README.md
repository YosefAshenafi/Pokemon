...
50 unit tests cover the formatting utilities (heights, weights, ids, names, effect text), the theme helpers (type colors, contrast, stat colors), the PokeAPI client (URL building, pagination, 404/network error handling — with `fetch` mocked at the network boundary), the type-index build (slot ordering, partial failures, total failure, progressive results), and component rendering/interaction for `PokemonCard` and `StatBar` via React Testing Library.
...
src/
  ...
  api/                 # Typed PokeAPI client, response types, query keys
  hooks/               # React Query hooks (list, detail, move, search, type index)
